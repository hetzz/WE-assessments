class Impurity{
    ...
    public static int impurity_count(String sentence){
        ...
        double [] impurity_increment = {0, 0.5, 0.7 ,1, 3};
        for (int i = 0; i < 26; i++){
            if(new String(vowels).indexOf((char)(i + 97)) != -1){
                if(letter_array[i] == 2) 
                    impurity_index += impurity_increment[1];
            }
            if(new String(vowels).indexOf((char)(i + 97)) == -1 && letter_array[i] == 2)
    ...
    public static void main(String args[]){
        String sentence = "Old brother fox jumps over the lazy dog.";
